...
6. `CostByTagWidget` — bar-list, top-N `opentrattos.tag` values (eligia cross-pollination, widget #7 NFR-OBS-10). Reads from `/m3/ai-obs/cost-by-tag` (separate endpoint because the GROUP BY is on a JSONB attr, slower).
...
Per-user widget config (order + visibility flags) lives in `localStorage` under key `opentrattos.aiObsDashboard.widgetConfig.v1`. The shape:
